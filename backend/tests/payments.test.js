...
const { Venda, Empresa, Usuario, Produto, ItemVenda, Pagamento, PagamentoLog } = require('../src/models');
...
describe('Payments API', () => {
  ...
  describe('GET /api/payments/status/:paymentId', () => {
    beforeEach(async () => {
      // Criar pagamento antes dos testes
      await request(app)
        .post('/api/payments/create')
        .set('Authorization', `Bearer ${authToken}`)
        .send({
    ...
    it('deve retornar status do pagamento', async () => {
      // Buscar o paymentId criado
      const vendaAtualizada = await Venda.findByPk(venda.id);
      const paymentId = vendaAtualizada.gateway_payment_id;
    ...
    it('deve retornar erro para pagamento não encontrado', async () => {
      const response = await request(app)
        .get('/api/payments/status/PAGAMENTO_INEXISTENTE')
        .set('Authorization', `Bearer ${authToken}`);

      expect(response.status).toBe(404);
      expect(response.body.error).toBe('Pagamento simulado não encontrado');
    });
  });

  describe('POST /api/payments/approve/:paymentId', () => {
    beforeEach(async () => {
      // Criar pagamento antes dos testes
      await request(app)
        .post('/api/payments/create')
    ...
    it('deve aprovar pagamento com sucesso', async () => {
      ...
      expect(response.status).toBe(200);
      expect(response.body.success).toBe(true);
      expect(response.body.message).toBe('Pagamento aprovado com sucesso!');
      expect(response.body.venda.status).toBe('finalizada');
      expect(response.body.venda.pago_em).toBeTruthy();
    });

    it('deve retornar erro para pagamento não encontrado', async () => {
      const response = await request(app)
        .post('/api/payments/approve/PAGAMENTO_INEXISTENTE')
        .set('Authorization', `Bearer ${authToken}`);
  ...
  describe('POST /api/payments/reject/:paymentId', () => {
    beforeEach(async () => {
      // Criar pagamento antes dos testes
      await request(app)
        .post('/api/payments/create')
    ...
    it('deve rejeitar pagamento com sucesso', async () => {
      ...
      expect(response.status).toBe(200);
      expect(response.body.success).toBe(true);
      expect(response.body.message).toBe('Pagamento rejeitado!');
      expect(response.body.motivo).toBe('Cliente cancelou');
      expect(response.body.venda.status).toBe('cancelada');
    });

    it('deve rejeitar pagamento sem motivo', async () => {
      // Buscar o paymentId criado
      const vendaAtualizada = await Venda.findByPk(venda.id);
      const paymentId = vendaAtualizada.gateway_payment_id;
      ...
      expect(response.status).toBe(200);
      expect(response.body.motivo).toBe('Pagamento cancelado pelo usuário');
    });
  });
  ...
  describe('POST /api/payments/simulate/:scenario', () => {
    ...
    it('deve simular cenário timeout', async () => {
      const response = await request(app)
        .post('/api/payments/simulate/timeout')
        .set('Authorization', `Bearer ${authToken}`)
        .send({
          vendaId: venda.id
        });
      ...
      expect(response.status).toBe(200);
      expect(response.body.scenario).toBe('timeout');
      expect(response.body.status).toBe('timeout');
      expect(response.body.delay).toBe(10000);
    });
  ...
  describe('Teste de Integração - Fluxo Completo', () => {
    it('deve executar fluxo completo de pagamento', async () => {
      // 1. Criar pagamento
